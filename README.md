...
| 4 | **Continuum Shift** (`/continuum`) — the product moat | Press **Try the winning demo phrase** (or use the microphone), confirm the extracted signals and Protected Constants, press **Compose the shift**, then apply | Voice or text turns one real-life change into the smallest coordinated decision set across Today, Week, Shopping and Pantry. The verified before/after Change Ledger is visible and reversible; audio is not stored. |
